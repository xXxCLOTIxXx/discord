...
class Requester:
	...
	def make_request(self, method: str, endpoint: str, body: dict = None, allowed_code: int = 200):

		response = self.session.request(method, f"{self.web_api}{endpoint}", data=dumps(body), headers=self.headers())
	...
	async def make_async_request(self, method: str, endpoint: str, body: dict = None, allowed_code: int = 200):
		async with ClientSession() as asyncSession:
			response = await asyncSession.request(method, f"{self.web_api}{endpoint}", data=dumps(body), headers=self.headers())
